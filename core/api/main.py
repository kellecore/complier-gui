...
def compile_endpoint(req: CompileRequest):
    ...
    local_hybrid = _resolve_hybrid_compiler(
    ...
    if req.v2:
        ...
        try:
            worker_res = local_hybrid.compile(req.text)
...
async def optimize_endpoint(req: OptimizeRequest):
    ...
    worker = _resolve_hybrid_compiler(
        llm_provider=req.llm_provider,
        llm_api_key=req.llm_api_key,
        llm_base_url=req.llm_base_url,
        llm_model=req.llm_model,
...
def validate_endpoint(req: ValidateRequest):
    ...
    try:
        report = _resolve_hybrid_compiler(
            llm_provider=req.llm_provider,
            llm_api_key=req.llm_api_key,
            llm_base_url=req.llm_base_url,
            llm_model=req.llm_model,
        ).worker.analyze_prompt(req.text)
        return report
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def fix_endpoint(req: AutoFixRequest):
    ...
    try:
        result = _resolve_hybrid_compiler(
            llm_provider=req.llm_provider,
            llm_api_key=req.llm_api_key,
            llm_base_url=req.llm_base_url,
            llm_model=req.llm_model,
        ).worker.fix_prompt(req.text)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
