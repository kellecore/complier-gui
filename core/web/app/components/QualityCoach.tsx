...
export default function QualityCoach({ prompt, onUpdatePrompt, llmConfig }: QualityCoachProps) {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            const res = await fetch("http://127.0.0.1:8080/validate", {
                ...
                body: JSON.stringify({
                    text: prompt,
                    llm_provider: llmConfig?.provider || null,
                    llm_api_key: llmConfig?.apiKey || null,
                    llm_base_url: llmConfig?.baseUrl || null,
                    llm_model: llmConfig?.model || null,
                }),
            });
            const data = await res.json();
            if (!res.ok) {
                throw new Error(data?.detail || `Validate failed (${res.status})`);
            }
            setReport(data);
        } catch (e) {
            console.error(e);
    ...
    const handleAutoFix = async () => {
        ...
        try {
            ...
            const data = await res.json();
            if (!res.ok) {
                throw new Error(data?.detail || `Fix failed (${res.status})`);
            }
            setFixResult(data);
        } catch (e) {
            console.error(e);
