...
export default function Sidebar() {
  ...
  const navItems = [
    ...
    { name: t("Offline", "Cevrimdisi"), path: "/offline", icon: "F" },
  ];
  ...
  return (
    <div className="w-16 md:w-20 h-screen bg-black/20 border-r border-white/5 flex flex-col items-center py-6 gap-6 backdrop-blur-md z-50">
      ...
      {navItems.map((item) => {
        ...
        return (
          <Link
            key={item.path}
            href={item.path}
            className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-semibold transition-all duration-300 relative group ${
              isActive
          ...
          >
            {item.icon}
            {isActive && <div className="absolute left-[-2px] top-2 bottom-2 w-1 bg-blue-500 rounded-full" />}
      ...
      <button
        ...
        className="mt-auto w-10 h-10 rounded-xl border border-white/10 bg-white/5 text-[11px] font-semibold text-zinc-300 hover:bg-white/10 transition-all"
        title={t("Switch Language", "Dili Degistir")}
      >
        {lang.toUpperCase()}
      </button>
    </div>
